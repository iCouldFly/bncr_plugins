...
module.exports = async s => {
    ...
    let input = await s.waitInput(async () => { }, 60)
    if (!input) {
        // 超时
        await s.delMsg(loading0)
        return s.reply("退出成功")
    }
    const _t = input.getMsg();
    if (isNaN(Number(_t)) || _t < 1 || _t > mainList.length) {
        await s.delMsg(loading0)
        return s.reply("输入有误,退出成功")
    }
    const index = +_t - 1;
    const _item = mainList[index];
    await s.delMsg(loading0)

    var _m = "";
    switch (mainList[index]) {
        case '京东命令':
            _m = [
            '登录	-	登录/更新京东账号', 
            '查询	-	查询京东资产', 
            ...
            '[pin;wskey;]	-	发送wskey上车']
            let loading1 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
        ...
        case '福利命令':
            ...
            '电信查询	-	查询88元电信车位的中奖记录']
            let loading2 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
        ...
        case '社交命令':
            ...
            let loading3 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
        ...
        case '查询命令':
            ...
            '百度百科[关键词]	-	查询百度百科', 
            '菜单	-	私人订制菜单']
            let loading4 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
        ...
        case '影音命令':
            ...
            '[快手视频链接]	-	快手视频解析', 
            'mv[曲名]	-	点歌MV版']
            let loading5 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
            await s.delMsg(loading5, { wait: 60 });
            break;
        case '游戏命令':
            _m = [
            '[亚瑟]战力	-	王者荣耀国标最低战力查询，四平台可查']
            let loading6 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
        ...
        case '随机命令':
            ...
            '来个帅哥	-	发送帅哥视频', 
            '我想睡觉了	-	有声小说']
            let loading7 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
        ...
        case '系统命令':
            ...
            '环境变量 状态' ]
            let loading8 = await s.reply(mainList[index] + "\n----------------------\n" + singleMenu(_m))
